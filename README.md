...
个人/团队用的 **AI 资讯自动化流水线**：从 400+ RSS、GitHub Trending、Hacker News 抓取内容，经 LLM 评分与摘要，通过 **企业微信群机器人** 推送热点与每日 digest，并在 **GitHub Pages** 托管可读全文。
...
| 能力 | 说明 |
|------|------|
| **Hourly Fetch** | 每小时抓取 RSS、LLM 评分；≥90 分即时推企微 |
| **每日 Digest** | 北京 ~08:05 生成早报（RSS + GH + HN + 洞察）；0–N 条；四段全空则静默并留痕 |
| **Pages 全文** | digest 生成后 **同 job 部署 Pages**，URL 200 后再发企微（避免 404 链接） |
| **健康检查** | 北京 ~09:00 检查当日 digest 是否执行，异常企微告警 |
| **幂等与状态** | `run-state.json` / `push-skip-*.json` 记录运行状态，防重复生成 |
...
| 文件 | 触发 | 作用 |
|------|------|------|
| `fetch.yml` | 每小时 | 抓取 + 评分 + 热点企微 + `commit-fetch` |
| `daily.yml` | 每天 08:05 UTC+8 附近 | 补抓 → digest → publish → **内联 deploy Pages** → 企微 |
| `health-check.yml` | 每天 09:00 | 检查当日 digest / skip 记录 |
...
| `news-data/push-*.md` / `.html` | 日报正文（驱动 Pages） |
| `news-data/run-state.json` | 最近 fetch / digest 状态 |
| `index.html` | Pages 索引（`build_pages_index.py` 生成） |

线上阅读示例：[AI Daily 站点](https://zk1239520941.github.io/ai-daily/)
...
| 项目 | 公开仓库（当前） |
|------|------------------|
| GitHub Actions | 标准 Linux runner **基本免费** |
| GitHub Pages | **免费** |
| DeepSeek API | 按 token 计费（主要成本） |
| 企微 Webhook | 免费 |

若改为 **Private** 仓库，Actions 有每月 2000 分钟限额，hourly fetch 可能较快触顶，需减频或升级计划。
