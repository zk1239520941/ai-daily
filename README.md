...
- 📬 **每日早报** —— 每天 08:00 一次 digest（0–N 条均可）；含 GitHub / HN / 跨板块洞察；四段全空则静默
...
- `dnews-fetch.service`（默认每 60 分钟）：抓取 RSS → 评分 → 命中 ≥90 分立即推送；fetch 数据 commit 到 git
- `dnews-push.service`（按 `push_cron`，默认每天 08:00）：生成 digest（RSS + GitHub + HN + 洞察）；无内容则跳过
...
```json
{
    ...
    "filter": {
        ...
        "push_window_hours": 24,  // digest 收录窗口（小时）
        "digest_min_items": 0,  // digest 最少 RSS 条数，0 表示不硬凑
        "skip_empty_digest": true,  // 四段全空时静默，不 save/publish/企微
        "exclude_notified_links_from_digest": true,  // 已即时推送的 link 不再进 digest
...
| `push_window_hours` | number | digest 收录窗口（小时），与上次 push 时间取 max（默认24） |
| `digest_min_items` | number | digest 最少 RSS 条数，0 表示不硬凑（默认0） |
| `skip_empty_digest` | boolean | 四段全空时跳过 save/publish/企微（默认 true） |
| `exclude_notified_links_from_digest` | boolean | 已即时推送的 URL 不再进入 digest（默认 true） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `fetch_interval_minutes` | number | RSS 抓取间隔，单位分钟（默认60分钟） |
| `fetch_lookback_minutes` | number | RSS 冗余缓存时间（分钟），必须大于 `fetch_interval_minutes`，用于防止 RSS 延迟导致漏读（默认120分钟） |
| `push_cron` | array | 每日 digest 的 cron 表达式。推荐只配一条（如 `0 8 * * *`）；每次 push 均含 GitHub / HN / 洞察三段 |
